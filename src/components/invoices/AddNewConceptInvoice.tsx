...
export default function AddNewConceptEstimate({showForm, updateConcepts, user, token, 
  conceptsDataChart, company}: {showForm:Function, updateConcepts:Function, user:string, 
    token:string, conceptsDataChart:IConceptEstimate[], company:string}) {
  ...
  const dataComp = conceptSel? <DataStepperComponent conceptSelected={conceptSel} token={token} 
                previousStep={handleIndexStepper} price={idPrice} user={user} idEstimate={' '}
    ...
    <>
      <form className="z-10 absolute top-16 w-full max-w-xl bg-white space-y-5 p-3 right-0"
        ...
        <div className="flex justify-between">
          <HeaderForm img="/img/estimates/concepts.svg" subtitle="Modifica y agrega mas conceptos a una factura" 
            title="Agregar conceptos a factura"
          />
