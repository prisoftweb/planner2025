...
const ListData = ({data, token}: {data: WorkflowTable[], token:string}) => {
    ...
    <div>
      <div className="relative flex flex-col text-gray-700 bg-white shadow-md w-full rounded-xl bg-clip-border] h-[calc(100vh-222px)]">
        <nav className="flex w-full flex-col gap-1 p-2 font-sans text-base font-normal text-blue-gray-700
          ...
          {filterData.map((c) => (
            <CardWorkflow workflow={c} key={c.id} token={token} />
          ))}
...
const CardWorkflow = ({workflow, token}: 
  {workflow:WorkflowTable, token:string}) => {
    ...
    >
      <div className="flex items-center w-full ">
        <div className="grid mr-4 place-items-center">
          ...
          <DeleteElement id={workflow.id} name={workflow.title} remove={removeWorkFlow} token={token} />
        </div>
