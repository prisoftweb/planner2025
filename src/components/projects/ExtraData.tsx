...
export default function ExtraData({token, optClients, optCategories, 
                          optTypes, optCompanies, id, project}:
                        {token:string, optClients:Options[], optCategories:Options[], 
                          optTypes:Options[], optCompanies: Options[], id:string,
                          project:Project}){
  ...
  let idCompany = 0;
  optTypes.map((optComp, index:number) => {
    if(optComp.value === project.company._id){
      idCompany = index;
    }
  });
  ...
  //   if(optCat.value === project){
  //     idType = index;
  //   }
  // })

  const [startDate, setStartDate] = useState<string>(project.datets);
  ...
  return(
    <div className="w-full">
      ...
      <form onSubmit={formik.handleSubmit} className="mt-4 max-w-sm rounded-lg space-y-5">
        ...
        <SelectReact opts={optCompanies} setValue={setCompany} index={idCompany} />
