...
import { GetCostsGroupByCostoCenterCategory, getAllCostsGroupByCOSTOCENTERCategoryByDate } from "@/app/api/routeCost";
...
export default function TableReportByCategory({token, company}: {token:string, company:string}){
  ...
  const fetch = async (dateIni:string, dateFinal:string) => {
    const res = await getAllCostsGroupByCOSTOCENTERCategoryByDate(token, dateIni, dateFinal);
...
const ListData = ({data, token}: {data: ReportByCostcenterCategory[], token:string}) => {
...
const CardData = ({data, token}: 
  {data:ReportByCostcenterCategory, token:string}) => {
