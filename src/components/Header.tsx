...
const ResponsiveHeaderClient = forwardRef<HTMLInputElement, HeaderProps>(function Header( 
  { children, title, placeHolder, other=false, hideChildren=false }, ref ){
  ...
  return(
    <>
      <div className="flex justify-between items-center gap-x-5 gap-y-3 flex-wrap sm:flex-nowrap">
        ...
        <div className="mt-2 md:mt-0 sm:flex gap-x-3 gap-y-2 pr-4 sm:pr-0 justify-end w-full">
          <SearchInTable placeH={placeHolder} ref={ref} />
          <div className={`w-76 mt-2 sm:mt-0 hidden ${hideChildren? '': 'sm:block'}`}>
            {children}
          </div>
        </div>
      </div>
