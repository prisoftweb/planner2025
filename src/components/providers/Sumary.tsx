...
export default function Sumary({provider, token, costPayment}:
  {provider:Provider, token:string, costPayment:ICostTOTALPendingPAYGroupByPROVIDER}){
      ...
      <div className="flex justify-center flex-wrap gap-y-2 mt-3">
        <div className="w-72 p-1">
          <Card p1={'SALDO ACTUAL' + ' ( ' + 
            ...
            // p2={"$" + provider.tradeline.currentbalance?.toLocaleString('en')}
            p2={MoneyFormatter(costPayment?.totalCost ?? 0)} 
        ...
        <div className="w-72 p-1">
          <Card p1="SALDO DISPONIBLE" 
            // p2={"$"+ ((provider.tradeline?.creditlimit || 0) - (provider.tradeline?.currentbalance || 0)).toLocaleString('en')} 
            p2={MoneyFormatter((provider?.tradeline?.creditlimit || 0) - (costPayment?.totalCost ?? 0))} 
        ...
        <div className="w-72 p-1">
          <Card p1="LINEA DISPONIBLE" 
            // p2={"$" + provider.tradeline.creditlimit?.toLocaleString('en')}
            p2={MoneyFormatter(provider?.tradeline?.creditlimit?? 0)}
