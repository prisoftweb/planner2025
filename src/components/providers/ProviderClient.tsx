...
export default function ProviderClient({provider, token, id, costPayment, user, company, permissions}: 
  {provider:Provider, token:string, id:string, costPayment:ICostTOTALPendingPAYGroupByPROVIDER, 
    user:string, company:string, permissions:IPermissionsAndComponents}){
  ...
  const databasic=permissions.components.includes('basicinfo')? <> <DataBasic id={id} provider={provider} token={token} user={user} />
                <div className="bg-white rounded-lg shadow-md pl-2 px-3 w-full max-w-md">
                  <ShowContactasProv provider={provider} token={token} />
                </div></>: <></>;

  const creditline=permissions.components.includes('creditline')? <CreditLine provider={provider} id={id} token={token} />: <></>;
  const contacts=permissions.components.includes('contact')? <Contacts id={id} contacts={provider.contact || []} token={token} company={company} />: <></>;
  const bankdata=permissions.components.includes('bankdetails')? <BankData id={id} provider={provider} token={token} company={company} user={user} />: <></>;
  const configprovider=permissions.components.includes('deleteprovider')? <ConfigProvider provider={provider} status={provider?.status?? false} token={token} />: <></>;
  const sumary=permissions.components.includes('basicinfo')? <> <Sumary provider={provider} token={token} costPayment={costPayment} />
                          <div className="bg-white rounded-lg shadow-md pl-2 px-3 w-full max-w-md">
                            <ShowContactasProv provider={provider} token={token} />
                          </div></>: <></>;
    ...
    <>
      <div className={`md:hidden bg-white`}>
        <NavResponsive open={open} setOpen={setOpen} changeOption={setOpt} option={opt}
          tradeline={provider.tradeline?.creditlimit ? true: false} permissions={permissions} />
      </div>
      <div className={`flex`}>
