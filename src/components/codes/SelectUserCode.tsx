...
type Props={
  token:string,
  returnProvider: () => void
  handleUserSel: (value: string) => void
  size: number
}

export default function SelectUserCode({token, handleUserSel, returnProvider, size}: Props) {

  const [heightPage, setHeightPage] = useState<number>(900);
  const [users, setUsers] = useState<UsrBack[]>([]);
  ...
  useEffect(() => {
    const fetch = async () => {
      const us = await getUsersMin(token);

      if(typeof(us)==='string'){
        showToastMessageError(us);
        return
      }
  ...
  const filteredUsers= search==''? users: users.filter((p) => p.name.toString().toLowerCase().includes(search.toLowerCase()));
