...
export default function ClientCli({client, token, id, tags}: 
  ...
  const [view, setView] = useState<JSX.Element>
                (<div className="w-full p-2 md:w-1/2 bg-white rounded-lg shadow-md
    ...
    opt===2? setView(<div className="mt-3 w-full max-w-md bg-white rounded-lg shadow-md pl-2 px-3" 
      ...
      (opt===3? setView(<div className="mt-3 w-full max-w-md border-t-8 lg:border-t-0 lg:border-l-8 
        ...
        (opt===4? setView(<div className="mt-3 w-full max-w-md border-t-8 lg:border-t-0 lg:border-l-8 
          ...
          (opt===5? setView(<div className="mt-3 w-full max-w-md border-t-8 lg:border-t-0 lg:border-l-8 
                      pl-2" style={{borderColor:'#F8FAFC'}}>
                        <Contacts id={id} contacts={client.contact || []} token={token} />
                      </div>):  setView(<div className="w-full p-2 md:w-1/2 border-t-8 lg:border-t-0 lg:border-l-8 
                                  pl-2" style={{borderColor:'#F8FAFC'}}>
                                    <Sumary client={client} idCli={id} token={token} />
                                </div>)) ))
  }, [opt, ])
